    ...
    <div class="background">
            ...
            <el-form ref="configForm" hide-required-asterisk size="small" label-position="top" :model="connection">
                <el-row :gutter="20">
                    ...
                    <el-col :span="24">
                        ...
                        <el-button v-if="client.connected" type="danger" size="small" class="conn-btn"
        ...
        <el-row>
            <el-col :span="20" :offset="2">
                <!-- 卡片样式更新：增加圆角和阴影 -->
                <el-card title="Received Data"
                    style="margin-top: 50px; height: 600px; border-radius: 15px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
                    <h2>Received Data</h2>

                    <div style="max-height: 450px; overflow-y: scroll;">
                        <el-timeline>
                            <el-timeline-item v-for="(msg, index) in receivedMessages" :key="index"
                                :timestamp="msg.timestamp" placement="top">
                                <el-card style="margin-bottom: 20px;">
                                    <div class="text-item">
                                        <div><strong>Topic:</strong> {{ msg.topic }}</div>
                                        <div><strong>QoS:</strong> {{ msg.qos }}</div>
                    ...
                    <div style="text-align: right; margin-top: 10px;">
                        <el-button style="border-radius: 10px; background-color: green; color: white;"
                            type="primary">Receive</el-button>
        ...
        <el-row style="margin-top: 50px;">
            ...
            <el-col :span="5" :offset="2">
                <el-card title="Processed Data"
                    style="height: 600px; border-radius: 15px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
            ...
            <el-col :span="7" :offset="1">
                <el-card title="每日数据汇总"
                    ...
                    <div style="text-align: right; margin-top: 10px;">
                        <el-button style="border-radius: 10px; background-color: green; color: white; margin-left: 30px;"
                            type="primary" @click="onCalculateClick()">Calculate</el-button>
                    </div>
                </el-card>
            </el-col>
            ...
            <el-col :span="6" :offset="1">
                <el-card title="Predicted Data"
                    ...
                    <div style="text-align: right; margin-top: 10px;">
                        <el-button style="border-radius: 10px; background-color: green; color: white; margin-left: 30px;"
                            type="primary">Predict</el-button>
                    </div>
                </el-card>
            </el-col>
        </el-row>
        ...
        <el-row style="margin-top: 50px;">
            <!-- 数据折线图 -->
            <el-col :span="20" :offset="2">
                <el-card title="Predicted Data Line Chart"
                    style="height: 500px;border-radius: 15px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
                    <h3>Predicted Data Line Chart</h3>

                    <div ref="lineChart"
                        style="width: 100%; height: 350px; border-radius: 10px; box-shadow: 0 2px 4px 0 rgba(0,0,0,0.1);">
                    </div>
                    <div style="text-align: right; margin-top: 10px;">
                        <el-button style="border-radius: 10px; background-color: green; color: white; margin-left: 30px;"
                            type="primary">Refresh</el-button>
        ...
        <TemperaturePrediction :summaryDataProp="summaryData" @childUpdatePredictedData="childUpdatePredictedData">
        ...
        <el-card class="box-card" style="margin-top:50px;margin-left: 150px;width: 1350px;border-radius: 15px;">
            ...
            <el-form ref="publish" hide-required-asterisk size="small" label-position="top" :model="publishData">
                ...
                placeholder="Enter message to publish">
                    ...
                    <el-input type="text" v-model="publishData.publishMessage"
...
<!-- 第一个图表 -->
<el-card class="box-cardone" shadow="always" style="margin-top:50px;margin-left: 150px;width: 80%; height: 500px; border-radius: 15px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
  <h3>Processed Data Line Chart</h3>
  <div id="processedDataChart-container" style="width: 1000px; height: 400px;"></div>
</el-card>

<!-- 第二个图表 -->
<el-card class="box-cardtwo" shadow="always" style="margin-top:50px;margin-left: 150px; width: 80%; height: 500px; border-radius: 15px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
  <h3>Summary Data Line Chart</h3>
  <div id="summaryDataChart-container" style="width: 100%; height: 400px;"></div>
</el-card>
</div>
...
export default {
    name: 'DataProcessing',
    components: {
    TemperaturePrediction, // 注册组件，使其在模板中可以使用
  },
    ...
    mounted() {
        ...
        this.initProcessedDataChart();
      this.initSummaryDataChart();
      this.formatPredictedData();
        this.initPredictedChart();
    ...
    watch: {
      processedData: {
      handler() {
        this.updateProcessedDataChart();
      },
      deep: true
    },
    summaryData: {
      handler() {
        this.updateSummaryDataChart();
      },
      deep: true
    }
  },
    methods: {
      ...
      this.formattedPredictedData = this.predictedData.flatMap(data => {
        const { date, predictedTemperature } = data
        return {
          date,
          value: predictedTemperature
        }
      }
        /**
        ...
        initPredictedChart() {
          this.predictedInstance = echarts.init(this.$refs.lineChart);

      //     console.log("initPredictedChart");
      //     const dom = document.getElementById('summaryDataChart-container');
      // this.summaryDataChart = echarts.init(dom);
      // this.updateSummaryDataChart(); 
           },
        updatePredictedData(predictedTemperature) {
            console.log('Received Predicted Temperature:', predictedTemperature);
            this.predictedData = predictedTemperature;
        },
        childUpdatePredictedData(data) {
        ...
        updateChartData() {
                  // 整理并处理数据
      const sortedData = this.predictedData.map(item => [item.date, item.predictedTemperature])
      ...
      console.log(sortedData)
           ...
           // const sortedData = [].concat.apply([], this.predictedData.map(item => Object.entries(item)))
            ...
            const formatDate = (dateStr) => {
                ...
                const time = dateStr.includes('T') ? dateStr.split('T')[1] : '';                    // 获取时间
            ...
            // 设置图表选项
            this.predictedInstance.setOption(option);
        },
        createConnection() {
            // 构建连接URL
        ...
        initProcessedDataChart() {
          console.log("initProcessedDataChart");

      const dom = document.getElementById('processedDataChart-container');
      this.processedDataChart = echarts.init(dom);
      this.updateProcessedDataChart();
    },
    updateProcessedDataChart() {
      if (!this.processedDataChart) {
    // 未初始化，可以选择重新初始化或返回
    return;
  }
  const option = {
    xAxis: {
      type: 'category',
      // 使用 date 和 time 属性来构建横坐标
      data: this.processedData.map(item => `${item.date} ${item.time}`),
    },
    yAxis: {
      type: 'value',
    },
  ...
  this.processedDataChart.setOption(option);
},
initSummaryDataChart() {
  console.log("initSummaryDataChart");

      const dom = document.getElementById('summaryDataChart-container');
      this.summaryDataChart = echarts.init(dom);
      this.updateSummaryDataChart();
    },
    updateSummaryDataChart() {
      if (!this.summaryDataChart) {
    // 未初始化，可以选择重新初始化或返回
    return;
  }
      const option = {
        xAxis: {
          type: 'category',
          data: this.summaryData.map(item => item.date)
        },
        yAxis: {
          type: 'value'
        },
        series: [
          {
            name: '最大值',
            data: this.summaryData.map(item => item.max),
            type: 'line'
          },
          {
            name: '最小值',
            data: this.summaryData.map(item => item.min),
            type: 'line'
          },
          {
            name: '平均值',
            data: this.summaryData.map(item => item.avg),
            type: 'line'
          }
        ]
      };
      this.summaryDataChart.setOption(option);
    }
    }
}
</script>
