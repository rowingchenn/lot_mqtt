      ...
      <el-form ref="publish" hide-required-asterisk size="small" label-position="top" :model="publishData">
        ...
        <div>
          ...
          <el-table :data="tableData" tooltip-effect="dark" style="width: 100%" max-height="400">
            ...
            <el-table-column label="Situation" width="180">
...
export default {
  ...
  methods: {
    ...
    prepareData(){
      this.publishData.publishMessage = JSON.stringify(this.tableData[this.publishState].rawData);
      //this.publishData.publishMessage=this.tableData[this.publishState].rawData
      this.tableData[this.publishState].read = true; // Mark the row as read
      this.tableData[this.publishState].state='已发布';
      this.publishState++;
    },
    doPublish() {
      this.prepareData();
    ...
    detectDataType(file) {
      console.log("我在 detectDataType");
      console.log(file.name);
      console.log(this.publishData.publishTopic);
      // 判断文件内容是否包含关键字来确定数据类型
      if (file.name.includes('temperature')&&this.publishData.publishTopic.includes('temperature')) {
        console.log("temperature成功");
        return 'temperature';
      } else if (file.name.includes('humidity')&&this.publishData.publishTopic.includes('humidity')) {
        console.log("humidity成功");
        return 'humidity';
      } else if (file.name.includes('pressure')&&this.publishData.publishTopic.includes('pressure')) {
